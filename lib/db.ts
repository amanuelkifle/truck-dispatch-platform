...
// objects, which blows up ("Objects are not valid as a React child") the
// moment a page renders one directly - and TypeScript still thinks these
// fields are `string` because that's what our types declare them as. Force
// DATE, TIME, TIMESTAMP, and TIMESTAMPTZ columns to come back as the plain
// strings Postgres already sends on the wire instead. Nothing in this app
// does JS Date arithmetic on query results (date comparisons/filters happen
// in SQL), so this is safe everywhere.
